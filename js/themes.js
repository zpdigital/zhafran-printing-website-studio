...
function renderThemes(){
  ...
  themes.innerHTML = filtered.map(item=>`
    <article class="card reveal show">
      <div class="theme-img">
        <img src="${themeImage(item)}" alt="${themeName(item)}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\'empty-thumb\\'>Thumbnail mengikuti API tema</div>'">
      </div>
  ...
  if(filtered.length === 0){
    themes.innerHTML = `<div class="card" style="grid-column:1/-1;text-align:center">Tema tidak ditemukan. Coba kata kunci atau filter lain.</div>`;
  }
}
